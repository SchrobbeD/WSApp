...
class WallStrikeGameView extends WatchUi.View {
    ...
    function onUpdate(dc as Graphics.Dc) as Void {
        ...
        for (i = 0; i < st.playerCount; i++) {
            ...
            if (st.systemId == 2) {
                label = label + " (" + st.lives[i] + "L)";
            }
            if (st.eliminated[i] != 0 || (st.systemId == 2 && st.lives[i] <= 0)) {
                label = label + " [OUT]";
            }
...
class WallStrikeGameDelegate extends WatchUi.BehaviorDelegate {
    ...
    function onTap(clickEvent as WatchUi.ClickEvent) as Boolean {
        ...
        st.gameRowFocus = idx;
        st.registerEliminationEvent(idx);
        WatchUi.requestUpdate();
        return true;
    }
    ...
    function onSelect() as Boolean {
        ...
        st.registerEliminationEvent(st.gameRowFocus);
        WatchUi.requestUpdate();
        return true;
    }
